 ...
 *
 */
public final class Pourcentages extends AbstractToStringImpl implements ImplShortCut
{
    /*---------- ATTRIBUTS ----------*/
    ...
    public void initialize()
    {
        ...
        {
            if (stat.getType() == TypeStatistique.POURCENTAGE)
                creerSerie(stat);
        }
    }
    ...
    private void creerSerie(StatistiqueEnum statEnum)
    {
        ...
        for (Statistique stat : liste)
        {
            ...
            {
                double valeur = stat.getValeur() / 100d;
    ...
    private String formatTexteNode(@SuppressWarnings("rawtypes") XYChart.Data data, StatistiqueEnum stat)
    {
        return "Date : " + Instant.ofEpochMilli((long) data.getXValue()).atZone(ZoneId.systemDefault()).toLocalDate() + "\n" + stat.getLabel() + " : " + ((Double) data.getYValue());
    }
